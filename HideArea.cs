/*
Lucas Ayres Student ID 2321346 
All code was created using the following sources:
 Pandemonium(4/12/2020) Unity 2D Platformer for Complete Beginners Playlist Accessed: 20/10/2024
 Game Maker's Toolkit(2/12/2022) The Unity Tutorial For Complete Beginners Accessed: 20/10/2024
 Brackey's(22/1/2017) How to make a Video Game - Getting Started (Unity) Accessed: 20/10/2024
 Stack Overflow Community. (2015). Spawning player at certain point in Unity [Online]. Stack Overflow. 
 Available from: https://stackoverflow.com/questions/31565355/spawning-player-at-certain-point-in-unity?rq=3 [Accessed 27 Oct 2024]
 Unity Documentation was also used but I didn't get anything the series did not cover. 
 ChatGTP was also used to grade Code for maintability and usability - no content generated only gave suggestions
*/

using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using TMPro;

public class HideArea : MonoBehaviour
{
    public Color hiddenColor = new Color(1f, 1f, 1f, 0.5f); //Turns player semi transparent for hiding
    public Color normalColor = Color.white; //Returns colour to the player
    private SpriteRenderer playerSpriteRenderer;//used to adjust player sprite
    private PlayerStealthState playerStealthState;//Sets if the player is hidden or not

    //When the player enters a stealth point they cannot be detected, this sets that
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            playerSpriteRenderer = other.GetComponent<SpriteRenderer>();
            playerStealthState = other.GetComponent<PlayerStealthState>();

            if (playerSpriteRenderer != null)
            {
                playerSpriteRenderer.color = hiddenColor;
            }

            if (playerStealthState != null)
            {
                playerStealthState.SetHidden(true); //Park player as hidden
            }

            Debug.Log("Player is hidden!");
        }
    }
    //Sets the player to be able to be detected
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            if (playerSpriteRenderer != null)
            {
                playerSpriteRenderer.color = normalColor;
            }

            if (playerStealthState != null)
            {
                playerStealthState.SetHidden(false); //Player as visible
            }

            Debug.Log("Player is visible!");
        }
    }
}